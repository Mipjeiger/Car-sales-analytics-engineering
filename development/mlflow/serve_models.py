...
class MLflowModelServing:
    ...
    def load_models(self):
        ...
        for model in registered_models:
            ...
            if "SalesPredictor" in model.name:
                ...
                self.models["sales"][model_key] = {"model": loaded_model, "version": latest_version}
    ...
    def predict_quantity(self, features, model_name="XGBoost"):
        ...
        if (
            model_name not in self.models["quantity"]
            or not self.models["quantity"][model_name]["model"]
        ):
            raise ValueError(f"Model {model_name} not found")
