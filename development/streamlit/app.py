...
def init_feature_extractor():
    ...
    return {"device": device, "model": model, "feature_extractor": model, "transform": transform}
...
def predict_all_models(brand_info):
    ...
    for model_name, model in available_sales_models.items():
        results["sales"][model_name] = predict_with_model(
            model, sales_features_df, model_type="sales"
        )

    qty_features_df = prepare_qty_features_for_prediction(brand_info)
    for model_name, model in available_qty_models.items():
        results["quantity"][model_name] = predict_with_model(
            model, qty_features_df, model_type="quantity"
        )
    ...
    for idx, score in zip(indices[0], distances[0]):
        if idx < len(feature_data):
            ...
            if len(brand_sales) > 0:
                ...
                result["top_model_qty"] = int(
                    brand_sales[brand_sales["model"] == top_model]["quantity"].sum()
                )
            ...
            if price_range:
                ...
                if result["avg_price"] and (
                    result["avg_price"] < min_price or result["avg_price"] > max_price
                ):
                    continue
...
def render_empty_state_placeholders():
    ...
    st.info(
        "👈 **Get Started:** Upload a vehicle image from the sidebar control panel to trigger live predictions!"
    )
...
def render_dev_overlay(models, brand_info=None):
    with st.expander("🛠️ DEVELOPER DEBUG PANEL", expanded=True):
        tab1, tab2, tab3 = st.tabs(
            ["📁 Model Load Status", "🔢 Feature Scaling Inspector", "🖼️ CV Vector Info"]
        )
        ...
        with tab1:
            ...
            with col_s:
                ...
                st.write(
                    f"• `Sales Scaler`: {'✅ Loaded' if models['sales_scaler'] is not None else '❌ Missing'}"
                )
            ...
            with col_q:
                ...
                for k, v in models["qty_models"].items():
                    st.write(f"• `{k}`: {'✅ Loaded' if v is not None else '❌ Missing/Failed'}")
                st.write(
                    f"• `Qty Scaler`: {'✅ Loaded' if models['qty_scaler'] is not None else '❌ Missing'}"
                )
        ...
        with tab2:
            ...
            dummy_info = (
                brand_info
                if brand_info
                else {"avg_price": 250000000, "total_quantity": 50, "profit_margin": 0.15}
            )
    ...
    with st.sidebar:
        ...
        with col1:
            sales_model = st.selectbox(
                "Sales Model", options=list(available_sales_models.keys()), index=0
            )
        ...
        with col2:
            qty_model = st.selectbox(
                "Quantity Model", options=list(available_qty_models.keys()), index=0
            )
    ...
    for idx, (col, result) in enumerate(zip(cols, results)):
        with col:
            try:
                ...
                st.image(
                    "https://via.placeholder.com/200x150?text=No+Image", use_container_width=True
                )
            ...
            st.markdown(
                ...
                if score > 0.70
                    ...
                    f"🟠 Match Similarity: **{score:.2%}**"
                    if score > 0.50
                    else f"🔴 Match Similarity: **{score:.2%}**"
                )
            )
...
def display_analytics_charts(results, sales_data, selected_sales_model):
    ...
    if valid_results:
        ...
        fig.add_trace(
            go.Bar(x=brands, y=actual_sales, name="Actual Sales", marker_color="#6366f1"),
            row=1,
            col=2,
        )
        fig.add_trace(
            go.Bar(
                x=brands,
                y=predicted_sales,
                name=f"Pred. ({selected_sales_model})",
                marker_color="#38bdf8",
            ),
            row=1,
            col=2,
    ...
    fig.update_layout(
        height=650, template="plotly_dark", showlegend=True, margin=dict(l=20, r=20, t=40, b=20)
    )
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("⚡ Executing Vision Search & Forecast Engine..."):
            results = get_recommendations(
                temp_path, k=k_results, price_range=price_range, selected_models=True
            )
        ...
        if results:
            ...
            display_recommendation_cards(
                results, show_sales, selected_sales_model, selected_qty_model
            )
